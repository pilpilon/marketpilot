...
assert.match(recorder, /waitForLoginOutcome/, 'verifier should wait for SPA/custom-token login outcomes, not only full-page navigation');
assert.match(recorder, /!accessibleApp && \(hasPasswordField/, 'verifier should not fail only because URL still contains login if the page already shows an authenticated app');
